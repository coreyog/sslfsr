...
// SSLFSR8 holds an 8 bit register
type SSLFSR8 struct {
	register byte
	interval int
	counter  int
...
func NewSSLFSR8(interval int) (sslfsr SSLFSR8) {
	return SSLFSR8{
		register: 1,
		interval: interval,
		counter:  0,
...
func (sslfsr *SSLFSR8) Shift() {
	reg := sslfsr.register
	bit := GetBit(reg, 0) != GetBit(reg, 2) != GetBit(reg, 3) != GetBit(reg, 4)
	reg = reg >> 1
	if bit {
		reg = reg | 0x80
	}
